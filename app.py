...
def load_sample_data():
    ...
    # Create a sample customer for demo purposes with password
    ...
    # Set a default password for the sample user: "password123"
...
def register_new_customer(system):
    st.markdown("<h2 class='subheader'>Customer Registration</h2>", unsafe_allow_html=True)
    ...
    with st.form("registration_form"):
        ...
        # Food preferences
        food_options = ["beef", "chicken", "lettuce", "tomato", "cheese", "bread", "milk", "eggs", "rice", "pasta"]
        favorite_food = st.multiselect("Favorite Foods", options=food_options)
        ...
        if submit_button:
            ...
            customer_id = f"C{len(system.customers) + 1}"
            ...
            try:
                system.register_customer(new_customer)
...
def login_customer(system):
    st.markdown("<h2 class='subheader'>Customer Login</h2>", unsafe_allow_html=True)
    ...
    with st.form("login_form"):
        ...
        if submit_button:
            ...
            customer = system.authenticate_customer(email, password)
            if customer:
                # Update the login timestamp and save
                customer.record_login()
                system.update_customer(customer)
                st.success(f"Welcome back, {email}!")
                
                # Store in session state
                st.session_state['logged_in'] = True
                st.session_state['current_user_id'] = customer.customer_id
                st.session_state['current_user_email'] = customer.email
...
def show_account_page(system):
    st.markdown("<h2 class='subheader'>My Account</h2>", unsafe_allow_html=True)
    
    customer = get_current_user(system)
    if not customer:
        st.warning("Please login to view your account details")
        login_customer(system)
        return
    ...
    with col1:
        ...
        st.write(f"**Birthdate:** {customer.birthdate.strftime('%Y-%m-%d')}")
        
        if customer.last_login:
            st.write(f"**Last Login:** {customer.last_login.strftime('%Y-%m-%d %H:%M:%S')}")
        
        favorite_html = ""
        for food in customer.favorite_food:
            favorite_html += f'<span class="badge badge-green">{food}</span>'
        st.markdown(f"**Favorite Foods:** {favorite_html}", unsafe_allow_html=True)
    ...
    with col2:
        ...
        if st.session_state.get('show_password_change', False):
            ...
            # Display receipt stats
            st.markdown("<div class='card'>", unsafe_allow_html=True)
            st.markdown("### Receipt Statistics")
            
            receipt_count = 0
            ingredient_count = 0
            expiring_soon = 0
            
            if customer.customer_id in system.receipts:
                receipts = system.receipts[customer.customer_id]
                ...
                for receipt in receipts:
                    unique_ingredients.update(receipt.ingredients)
                    days_remaining = (receipt.shelf_life - now).days
                    if 0 < days_remaining <= 3:
                        expiring_soon += 1
                
                ingredient_count = len(unique_ingredients)
            
            col1, col2 = st.columns(2)
            with col1:
                st.metric("Total Receipts", receipt_count)
                st.metric("Unique Ingredients", ingredient_count)
            with col2:
                st.metric("Expiring Soon", expiring_soon)
                st.metric("Saved Items", "12")  # Mock data for UI completeness
            ...
            if customer.customer_id in system.receipts and system.receipts[customer.customer_id]:
                for receipt in sorted(system.receipts[customer.customer_id], 
                                     key=lambda r: r.upload_date, reverse=True)[:3]:
                    st.markdown(f"""
                    <div style="display: flex; margin-bottom: 0.8rem; padding-bottom: 0.8rem; border-bottom: 1px solid #eee;">
                        <div style="width: 40px; text-align: center; margin-right: 10px;">
                            <img src="https://cdn-icons-png.flaticon.com/512/1611/1611154.png" width="30">
                        </div>
                        <div>
                            <div style="font-weight: bold;">Receipt {receipt.receipt_id} Uploaded</div>
                            <div style="font-size: 0.8rem; color: #777;">{receipt.upload_date.strftime('%Y-%m-%d %H:%M')}</div>
                        </div>
                    </div>
...
def show_receipt_upload(system):
    ...
    with col1:
        ...
        # Get current user or allow selection for admin
        current_user = get_current_user(system)
        if current_user:
            selected_customer = current_user
            st.info(f"Uploading receipt for: {selected_customer.email}")
        else:
            # Customer selection (admin mode)
...
def main():
    ...
    # Sidebar navigation with login/account status
    with st.sidebar:
        ...
        if is_logged_in():
            ...
            <div class="user-badge">
                ...
                <span>Logged in as: {st.session_state.get('current_user_email')}</span>
            </div>
            """, unsafe_allow_html=True)
        ...
        # Navigation options change based on login status
        if is_logged_in():
            page = st.radio(
                "Select a page",
            ...
            # If not logged in, show limited options or redirect to authentication
            auth_page = st.session_state.get('auth_page', None)
            if auth_page in ['login', 'register']:
                page = auth_page
            else:
                page = st.radio(
                    "Select a page",
                    ["Home", "Store Marketplace", "Login", "Register"]
                )
        
        st.divider()
        if st.button("Reset Demo Data", type="secondary"):
            st.session_state.pop('system_data', None)
            # Also clear login state when resetting
            if 'logged_in' in st.session_state:
                del st.session_state['logged_in']
            st.experimental_rerun()
    
    # Page content based on selection and login status
    if page == "Login" or (not is_logged_in() and page in ["Food Expiry Tracking", "Receipt Upload", 
                                                          "View Receipts", "Recommendations", "My Account"]):
        with st.container():
        ...
        with st.container():
            st.markdown("<div class='login-container'>", unsafe_allow_html=True)
            register_new_customer(system)
            st.markdown("</div>", unsafe_allow_html=True)
    elif page == "My Account":
        show_account_page(system)
    elif page == "Home":
